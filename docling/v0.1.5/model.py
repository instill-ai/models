...
class DocumentProcessor:
    ...
    async def __call__(self, request):
        ...
        for input_data in inputs:
            ...
            markdown_pages, extracted_images, pages_with_images, structured_document = (
                await self._process_document(input_data["data"]["doc_content"])
            )

            # Structure the output. `structured_document` is the DoclingDocument
            # export_to_dict() tree (schema_name == "DoclingDocument") — the M7
            # evidence tree the artifact parsing-router persists to
            # converted_file.evidence_tree for stable-anchor grounding. Emitting it
            # here brings the served model to parity with the host granite-docling
            # server (server/granite_docling.py) so the live + served paths agree.
            output = {
                "data": {
    ...
    async def _process_document(
        ...
        # The canonical DoclingDocument tree (export_to_dict) — schema-marked
        # (schema_name == "DoclingDocument"), the structured evidence tree the
        # artifact parsing-router persists for M7 stable-anchor grounding. Mirrors
        # the host granite-docling server's structured_document output.
        structured_document = result.document.export_to_dict()

        return markdown, extracted_images, pages_with_images, structured_document
